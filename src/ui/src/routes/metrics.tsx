...
// Recharts (~364 KB, incl. the shadcn chart wrapper that re-exports it) is
// pulled in ONLY through this lazy import, so it lands in a separate async
// chunk instead of the synchronous /metrics route chunk. It downloads the
// first time the metrics view renders, behind the Suspense fallback below.
const MetricsCharts = lazy(() => import("@/components/metrics/metrics-charts"))
...
import { MetricsRouteError } from "@/components/layout/metrics-route-error"
...
export const Route = createFileRoute("/metrics")({
  ...
  errorComponent: MetricsRouteError,
...
function MetricsView({ data }: { data: import("@/lib/types").Metrics }) {
  ...
  return (
    <>
      ...
      {/* Status breakdown + errors by route — recharts loads as a lazy chunk */}
